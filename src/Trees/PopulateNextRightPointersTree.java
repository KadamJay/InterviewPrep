...
// root:
// this pointer is initialized with the root of the tree, this pointer points to
// the prev level
...
// this acts as the head of the level and does not iterate trhough the nodes of
// the level, just
// points to the leftmost node of the level
// current:
// this pointer is used to iterate through the nodes of the current level i.e.,
// the level pointed by dummyhead and while iterating it populates the next
// pointers
...
public class PopulateNextRightPointersTree {
    ...
    public void usingLevelOrderWithoutQueue(TreeLinkNode root) {
        ...
        while (root != null) {
            ...
            // in the inner while loop, root becomes null when it reaches the
            // righmost_node.next of the level
            while (root != null) {
                if (root.left != null) {
