...
class CoinChangeIISolver {
    public int changeQuadratic(int totalAmount, int[] coins) {
        ...
        for (int i = 1; i <= noOfCoins; i++) {
            // whatever number of coins are available, sum 0 can be made only by excluding
            // all the
            // available coins hence it can be achieved in 1 way
            dp[i][0] = 1;
            // filling the DP table
            for (int j = 1; j <= amt; j++) {
                // if the value of coin is greater than the amount to be made, exclude the coin,
                // copy value
    ...
    public int changeLinear(int totalAmount, int[] coins) {
        ...
        // each cell of this dp represents, the number of ways in which amount i can be
        // made with given coins
        ...
        // we will take each coin and if the amount to be made is greater than the coin
        // value we will
...
/**
 * This problem is also called Integer Partition problem. Given integer, in how
 * many ways it can be represented
